...
class CyberportScraper(BaseScraper):
    ...
    async def _parse_search_results(self, url: str) -> list[TargetProduct]:
        try:
            ...
            for item in soup.select(".productList__item, .product-item, [data-product]"):
                name_el = item.select_one(".product-title, .productTitle, h2, h3")
                price_el = item.select_one(".price, .productPrice, [data-price]")
