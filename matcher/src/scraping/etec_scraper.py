...
class EtecScraper(BaseScraper):
    retailer_name = "E-Tec"
    base_url = "https://www.e-tec.at"

    async def search_by_ean(self, ean: str) -> list[TargetProduct]:
        url = f"{self.base_url}/search?q={ean}"
    ...
    async def _parse_search_results(self, url: str) -> list[TargetProduct]:
        try:
            ...
            for item in soup.select(".product-item, .productItem, [data-product]"):
                name_el = item.select_one(".product-title, .product-name, h2, h3")
