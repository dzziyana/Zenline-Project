...
- **Region suffix stripping**: Samsung encodes the sales region in model numbers (`QE55Q7FAAUXXN` in Austria vs `QE55Q7FAMTXZT` in Italy). We strip suffixes like `AUXXN`, `AAUXXN`, `MTXZT` to match the same product across markets. The regex for this: `(AU|AA|MT|AT)[A-Z]{2,4}$`
...
Multiple strategies may find the same (source, target) pair. We keep only the highest-confidence version. This is a simple dictionary keyed by `(source_reference, target_reference)` -- if a new match has higher confidence than the existing one, it replaces it.

### The Architecture

```
React Frontend (Vite + TypeScript)   <-- the web UI users interact with
    |
    v
FastAPI Backend (port 8001)          <-- Python web server handling all logic
    |-- /api/match/{category}     Run pipeline on demand
    |-- /api/search               Full-text product search (AND-matches multi-word queries)
    |-- /api/chat                 Natural language queries (Claude Haiku with DB context)
    ...
    |-- /api/dashboard            Real-time stats, method breakdown, confidence distribution
...
SQLite Database (data/matcher.db)
    |-- products      Source + target products with normalized brands, specs as JSON
    |-- matches       Every match with method, confidence, target metadata
    |-- scrape_results  Raw scraping output for auditing (URL, price, raw name)
...
3. **Brand filtering**: Pass `?brand_filter=Tefal` to only match products from a specific brand -- the jury asked about this "Only look for Brand X" scenario.
...
5. **Chat interface**: Natural language product queries powered by Claude Haiku. The chat endpoint searches the database, provides context to the LLM, and returns formatted responses. Falls back to smart keyword matching when no API key is available.
...
- **RESTful API** with 15+ endpoints, auto-generated interactive documentation at `/docs` (via OpenAPI/Swagger -- an industry standard for API documentation), optional API key authentication, and CORS support (allows the frontend to talk to the backend even when served from different ports)
