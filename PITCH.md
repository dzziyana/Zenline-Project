...
**On generalizability:** The taxonomy is designed to be extended, not rewritten. Adding a new product category (e.g. clothing, furniture) means adding keyword entries to a single Python dictionary -- no pipeline code changes. The keywords themselves are just product terms that anyone familiar with the domain would know ("Mikrowelle" = microwave, "Geschirrspueler" = dishwasher). In practice, when the Large Appliances category dropped mid-hackathon with 44 sources and 3,543 targets, we added ~15 new product types and had strong results within the hour. The classifier also has a specs-based fallback: if keywords don't match, it checks the product's `Produkttyp` specification field, which many retailers already provide. For truly unknown products, the other six pipeline stages (EAN, model number, fuzzy matching, scraping) still work -- product type matching is one signal among many, not the only one.
...
- **Semantic similarity search** using multilingual-e5-large-instruct -- a 560M parameter AI model that converts product names into numerical vectors (lists of numbers representing meaning). Products with similar meanings end up with similar vectors, even across German and English. Vectors are precomputed on a GPU server, then used for instant "find similar products" lookups via the API -- a supplementary discovery tool alongside the main matching pipeline.
