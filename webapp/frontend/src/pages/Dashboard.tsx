...
const STRATEGY_HINTS = [
  {
    condition: (ids: Set<string>) => ids.size === 0,
    textKey: "hint.all_off",
    icon: "~",
  },
  {
    condition: (ids: Set<string>) => ids.size === STRATEGIES.length,
    textKey: "hint.all_on",
    icon: ">",
  },
  {
    condition: (ids: Set<string>) => ids.has("ean") && ids.size === 1,
    textKey: "hint.ean_only",
    icon: "+",
  },
  {
    condition: (ids: Set<string>) => !ids.has("llm") && ids.size >= 3,
    textKey: "hint.no_llm",
    icon: "?",
  },
  {
    condition: (ids: Set<string>) => !ids.has("vision") && ids.has("embedding"),
    textKey: "hint.no_vision",
    icon: "*",
  },
  {
    condition: (ids: Set<string>) => ids.has("scrape") && !ids.has("ean"),
    textKey: "hint.scrape_no_ean",
    icon: "!",
  },
  {
    condition: (ids: Set<string>) => ids.size >= 2 && ids.size <= 4,
    textKey: "hint.nice_combo",
    icon: "~",
  },
];
...
const STRATEGY_ICONS: Record<string, JSX.Element> = {
  ...
  model_series: (
    ...
    >
      ...
      <line x1="22" y1="12" x2="28" y2="12" opacity="0.3" />
      <line x1="22" y1="16" x2="28" y2="16" opacity="0.3" />
    </svg>
  ),
  product_line: (
    <svg
      ...
      strokeLinejoin="round"
    >
      <rect x="4" y="12" width="10" height="16" rx="2" />
      <rect x="17" y="8" width="14" height="20" rx="2" />
      <line x1="9" y1="18" x2="9" y2="24" opacity="0.3" />
      <line x1="24" y1="14" x2="24" y2="24" opacity="0.3" />
      <path d="M14 20L17 20" strokeDasharray="2 2" />
    </svg>
  ),
  product_type: (
    <svg
  ...
  screen_size: (
    ...
    >
      ...
      <path d="M10 12L26 22" opacity="0.3" />
      <text
        x="11"
        y="21"
        fontSize="7"
        ...
        <div className="stats-grid">
          ...
          <div className="stat-card">
            ...
            <div className="stat-note">
              From {d?.retailers?.length ?? 0} retailers
            </div>
          </div>
          <div className="stat-card">
            ...
            <div className="stat-note">
              {d?.sources_matched ?? 0} {t("stats.sources_covered")}
            </div>
          </div>
          <div className="stat-card">
            <div className="stat-label">{t("stats.coverage")}</div>
            <div className="stat-value">
              {d ? `${d.coverage_pct.toFixed(0)}%` : "--"}
            </div>
            <div className="stat-note">
              {d && d.source_count > 0
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
              {d.brands.slice(0, 12).map((b) => (
                ...
                >
                  ...
                  <span
                    style={{ color: "var(--stone-500)", marginLeft: "8px" }}
                  >
          ...
          <div className="strategy-grid">
            {STRATEGIES.map((s) => {
              ...
              return (
                <div
                  key={s.id}
                  className={`strategy-card ${comingSoon ? "strategy-disabled" : enabled ? "strategy-enabled" : "strategy-disabled"}`}
                  data-strategy={s.id}
                ...
                >
                  <div className="strategy-header">
                    <div
                      className={`strategy-icon ${enabled && !comingSoon ? "" : "strategy-icon-dim"}`}
                    >
                      {STRATEGY_ICONS[s.id]}
                    </div>
                    {comingSoon ? (
          ...
          <div className="card">
            ...
            {categories.length === 0 ? (
              <div className="empty-state">
                ...
                <p>
                  Upload source and target data via the API or place JSON files
          ...
          <div className="card">
            ...
            {d && d.retailers.length > 0 ? (
              ...
              <div className="empty-state">
                ...
                <p>
                  Run the pipeline to discover retailers from target products.
                </p>
              </div>
            )}
          </div>
